...
6. The live model uses a normalized jet map (blue = lowest average, red = highest average, with cyan/green/yellow in between) and keeps orientation labels and intensities directly matched: front=inferior, back=posterior, left=lateral, right=medial, top=superior, bottom=anterior.
